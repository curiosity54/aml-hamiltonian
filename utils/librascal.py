...
#         print(actual_global_species)
        ...
        sparse = Labels(
            ...
            values=np.array(
                [
                    [l]#, center_species, neighbor_species]
                    for l in range(hypers["max_angular"] + 1)
                    #for center_species in actual_global_species
                    #for neighbor_species in actual_global_species
                ],
                dtype=np.int32,
            ),
        )
        ...
        for i, ijf in enumerate(ijframes):
            ...
            samples.append( np.asarray( [nonzero[0]*0+i, nonzero[0], nonzero[1],  numbers[nonzero[0]], numbers[nonzero[1]]]).T)
#             samples.append( np.asarray([nonzero[0]*0+i, nonzero[0], nonzero[1]]).T)
        
        data = np.concatenate(data)
        samples = Labels(
                names=["structure", "center_i", "center_j", "species_i", "species_j"],
                values=np.concatenate(samples).astype(np.int32)
        )
        blocks = []
        for (l,) in sparse: #center_species, neighbor_species) in sparse:
#             neighbor_species_i = actual_global_species.index(neighbor_species)
#             center_species_mask = np.where(info[:, 2] == center_species)[0]
#             block_data = data[center_species_mask, neighbor_species_i, :, lm_slices[l]]
#             block_data = block_data.swapaxes(1, 2)
...
#                 values=np.copy(info[center_species_mask, :2], info).astype(np.int32),
#             )
